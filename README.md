GDP (Docker + PHP5,7,8 + Go) 是一款全功能的**一键安装程序，支持Arm CPU**。

GDP项目说明：
1. 支持**HTTPS和HTTP/2**
2. **PHP源代码、MySQL数据、配置文件、日志文件**都可在Host中直接修改查看
3. 内置**完整PHP扩展安装**命令`install-php-extensions apcu`
...
2. `clone`项目：
    ```
    $ git clone https://github.com/amsterdan5/gdp.git
    ```
3. 如果主机是 Linux系统，且当前用户不是`root`用户，还需将当前用户加入`docker`用户组：
    ...
    ```
4. 在 .env 中指定你的代码路径
5. 拷贝并命名配置文件（Windows系统请用`copy`命令），启动：
    ```
    $ docker-compose up   # 启动
    ```
6. 在浏览器中访问：`http://localhost`或`https://localhost`(自签名HTTPS演示)就能看到效果。
...
### 3.5 使用composer
**方法1：主机中使用composer命令**
1. 确定composer缓存的路径。比如，我的gdp下载在`~/gdp`目录，那composer的缓存路径就是`~/gdp/data/composer`。
2. 参考[bash.alias.sample](bash.alias.sample)示例文件，将对应 php composer 函数拷贝到主机的 `~/.bashrc`文件。
    > 这里需要注意的是，示例文件中的`~/gdp/data/composer`目录需是第一步确定的目录。
3. 让文件起效：
    ```bash
    source ~/.bashrc
    ```
4. 在主机的任何目录下就能用composer了：
    ...
    ```
5. （可选）第一次使用 composer 会在 `~/gdp/data/composer` 目录下生成一个**config.json**文件，可以在这个文件中指定国内仓库，例如：
    ```json
    {
        "config": {},
        "repositories": {
